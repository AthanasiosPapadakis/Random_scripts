...
for line in f1:
    ...
    if int(data[0]) == 5:
        if differentvisits[25 - static_nodes - 1] == 0:
            # Store the first Static node and the first timestamp
            timestayed[25 - static_nodes - 1][differentvisits[25 - static_nodes - 1]] = data[1]
            # Store the first Static node and the first timestamp
            sequence[25 - static_nodes - 1][differentvisits[25 - static_nodes - 1]] = data[2]
            # Step
            differentvisits[25 - static_nodes - 1] = differentvisits[25 - static_nodes - 1] + 1
        ...
        elif int(data[2]) != sequence[25 - static_nodes - 1][differentvisits[25 - static_nodes - 1] - 1]:
            # Calculate the time sta25ed in this Static node
            timestayed[25 - static_nodes - 1][differentvisits[25 - static_nodes - 1] - 1] = int(data[1]) - timestayed[25 - static_nodes - 1][
                differentvisits[25 - static_nodes - 1] - 1]
            # We are using the next slot to store the previous timestamp:
            timestayed[25 - static_nodes - 1][differentvisits[25 - static_nodes - 1]] = int(data[1])
...
for i in range (0, static_nodes):
    if totalvisitsSN[i]!=0:
        timeaverage[i]= totaltimeSN[i]/totalvisitsSN[i]
        timeaverage[i]= round(timeaverage[i],1)
    else:
        timeaverage[i]= 0

df1 = pd.DataFrame(differentvisits, index=['MN1'], columns=["Visits"])
df1.to_csv('differentvisits1.csv')
df2 = pd.DataFrame(perfectchanges, columns=["MobileNode", "Timestamp", "StaticNode"])
df2.to_csv('perfectchanges1.csv')
df3 = pd.DataFrame(timestayed, index=['MN1'])
df3.to_csv('timestayed1.csv')
df4 = pd.DataFrame(sequence, index=['MN1'])
df4.to_csv('sequence1.csv')
df5 = pd.DataFrame(sumtimestayed, columns=["SN1", "SN2", "SN3", "SN4", "SN5", "SN6", "SN7", "SN8", "SN9", "SN10", "SN11", "SN12", "SN13", "SN14", "SN15", "SN16", "SN17", "SN18", "SN19", "SN20", "SN21", "SN22", "SN23","SN24"], index=['MN1'])
df5.to_csv('sumtimestayed1.csv')
df6 = pd.DataFrame(sumtimesvisited, columns=["SN1", "SN2", "SN3", "SN4", "SN5", "SN6", "SN7", "SN8", "SN9", "SN10", "SN11", "SN12", "SN13", "SN14", "SN15", "SN16", "SN17", "SN18", "SN19", "SN20", "SN21", "SN22", "SN23","SN24"], index=['MN1'])
df6.to_csv('sumtimesvisited1.csv')
...
for i in range (0, min_size-1):
    if (minimumrun[5][i]>=4180 and minimumrun[5][i+1]<=4180) or i==(min_size-2):
...
for i in range (0, medium_size-1):
    if (mediumrun[5][i]>=4180 and mediumrun[5][i+1]<=4180):
...
for i in range (0, max_size-1):
    if (maximumrun[5][i]>=4180 and maximumrun[5][i+1]<=4180):
...
for i in range (0, result[0][0]-1):
    if (averagerun[5][i]>=4100 and averagerun[5][i+1]==0):
