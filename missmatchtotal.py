...
#df13 = pd.DataFrame(totalmismatch, columns=['Real Beacons', 'Minmismatch', 'Minmismatchtotal', 'Medmismatch', 'Medmismatchtotal', 'Maxmismatch', 'Maxmismatchtotal', 'Avermismatch','Avermismatchtotal'])
#df13.to_csv('totalmismatch.csv')
...
for i in range (1, 9, 2):
    ...
    sd[j] = round(statistics.stdev(swapped_totalmismatch[i]),1)
    average[j] = round(mean(swapped_totalmismatch[i]),1)
    j = j + 1

# -------------------------- GUI START ------------------------------------

data1 = {'Minimum RR': average[0], 'Medium RR': average[1], 'Maximum RR': average[2], 'Average RR': average[3]}
annotate1 = [average[0], average[1], average[2], average[3]]
rr1 = list(data1.keys())
values1 = list(data1.values())

plt.bar(rr1, values1, color=['red', 'green', 'blue', 'cyan'])
plt.errorbar(rr1, values1, yerr = sd, fmt='o')

for i in range (0,4):
    plt.annotate(round(annotate1[i],1), xy=(rr1[i],values1[i]), ha='center', va='top')
    plt.annotate(round(sd[i],1), xy=(rr1[i],values1[i]), ha='center', va='bottom')
